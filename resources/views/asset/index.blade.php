...
<style>
    ...
    /* Style untuk cell yang bisa di-copy */
    .copyable-cell {
        cursor: pointer;
        position: relative;
    ...
    .copyable-cell:hover .copy-tooltip {
        opacity: 1;
    }
...
<div class="container-fluid py-4">
    <div class="d-flex flex-row flex-lg-row justify-content-between align-items-start align-items-lg-center mb-4 gap-3">
        <h5 class="mb-0" style="color: #06b6d4 !important;">
            <i class="fas fa-box-open" style="color: #06b6d4 !important;"></i> Daftar Asset
        </h5>
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <!-- Setiap cell dengan class copyable-cell -->
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->no_asset }}')">
                            <strong>{{ $asset->no_asset }}</strong>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="text-nowrap copyable-cell" onclick="copyToClipboard(this, '{{ $asset->latitude ?? '-' }}')">
                            <span title="{{ $asset->latitude ?? '-' }}">{{ Str::limit($asset->latitude ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="text-nowrap copyable-cell" onclick="copyToClipboard(this, '{{ $asset->longtitude ?? '-' }}')">
                            <span title="{{ $asset->longtitude ?? '-' }}">{{ Str::limit($asset->longtitude ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>

                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ addslashes($asset->nama_asset) }}')">
                            <span title="{{ $asset->nama_asset }}">{{ Str::limit($asset->nama_asset, 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->ketagori ?? '-' }}')">
                            <span title="{{ $asset->ketagori ?? '-'}}">{{ Str::limit($asset->ketagori ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->sub_kategori ?? '-' }}')">
                            <span title="{{ $asset->sub_kategori ?? '-'}}">{{ Str::limit($asset->sub_kategori ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->merek ?? '-' }}')">
                            <span title="{{ $asset->merek ?? '-'}}">{{ Str::limit($asset->merek ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->tipe ?? '-' }}')">
                            <span title="{{ $asset->tipe ?? '-'}}">{{ Str::limit($asset->tipe ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->nomor ?? '-' }}')">
                            <span title="{{ $asset->nomor ?? '-'}}">{{ Str::limit($asset->nomor ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="text-center copyable-cell" onclick="copyToClipboard(this, '{{ $asset->tahun ?? '-' }}')">
                            {{ $asset->tahun ?? '-' }}
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->lokasi ?? '-' }}')">
                            <span title="{{ $asset->lokasi ?? '-'}}">{{ Str::limit($asset->lokasi ?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->penanggung ?? '-' }}')">
                            <span title="{{ $asset->penanggung ?? '-'}}">{{ Str::limit($asset->penanggung?? '-', 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>

                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->kondisi ?? '-' }}')">
                        ...
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->status ?? '-' }}')">
                        ...
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->harga ? 'Rp ' . number_format($asset->harga) : '-' }}')">
                        ...
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ $asset->nilai ? number_format($asset->nilai) : '-' }}')">
                        ...
                        <td class="copyable-cell" onclick="copyToClipboard(this, '{{ addslashes($asset->user->name) }}')">
                            <span title="{{ $asset->user->name }}">{{ Str::limit($asset->user->name, 50) }}</span>
                            <span class="copy-tooltip">Klik untuk copy</span>
                        </td>
                        
                        <td class="text-nowrap copyable-cell" onclick="copyToClipboard(this, '{{ $asset->updated_at->format('d/m/Y H:i') }}')">
...
<!-- SweetAlert2 + Script Delete & Copy -->
...
function copyToClipboard(element, text) {
    // Jangan copy jika klik di kolom aksi
    if (element.querySelector('.btn-group')) {
        return;
    }
